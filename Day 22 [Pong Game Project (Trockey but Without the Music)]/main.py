...
# 2. Set positions, stretch factors, and territorial boundaries
...
# Now takes the full paddle object so it can check if it is active!
def handle_puck_collision(puck, paddle, pad_w, pad_h, player_name):
    pad_x = paddle.xcor()
    pad_y = paddle.ycor()
    dx = puck.xcor() - pad_x
    dy = puck.ycor() - pad_y
    ...
    if intersect_x > 0 and intersect_y > 0:
        # If the paddle is frozen, it acts as a dead wall and gets NO credit for the hit
        if paddle.is_active:
...
while game_is_on:
    ...
    # Penalty times bumped up to 5 seconds to ensure you feel the freeze!
    ...
    # ----------------------------------------------------
    if not handle_puck_collision(puck, l_paddle, 40, 240, "left"):
        if not handle_puck_collision(puck, r_paddle, 40, 240, "right"):
            handle_puck_collision(puck, t_paddle, 240, 40, "top")
