...
def register():
    ...
    if form.validate_on_submit(): #type:ignore
        ...
        user=result.scalar()
        if user:
            flash("You've already signed up, ano baka! Log in desu yo")
        ...
        new_user=User(
            ...
            password=hashed_and_salted_password, #type:ignore
        )
...
def contact():
    if request.method == "POST":
        ...
        with smtplib.SMTP("smtp.gmail.com") as connection:
            connection.starttls()
            connection.login(OWN_EMAIL, OWN_PASSWORD) #type:ignore
